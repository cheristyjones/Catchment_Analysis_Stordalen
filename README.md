# Code for the manuscript "Peatlands Control Aquatic Carbon Pools and Fluxes in an Arctic Subcatchment"
This repository contains data, scripts, and data processing for the manuscript "Peatlands Control Aquatic Carbon Pools and Fluxes in an Arctic Subcatchment", currently in review at JGR: Biogeosciences. This project investigates the spatial variability of aquatic C emissions and landscape connectivity throughout an Arctic subcatchment using DOM characterization and CO2 flux measurements. 

The data for the main analysis is depositied in Zenodo (https://doi.org/10.5281/zenodo.19581159). Please cite the dataset:

Jones, C. (2026). Peatlands Control Aquatic Carbon Pools and Fluxes in an Arctic Subcatchment [Data set]. Zenodo. https://doi.org/10.5281/zenodo.19581159

 ## Repositiory Structure
 ### /Data
 - Datasets used for analysis
 - ReadMe file contains a description of column variables in the datasets
 - Large files stored using GIT LFS

### /Scripts
   - Scripts used for analaysis, model runs, and figure creation
   - Named in order of analysis steps
  
   ### /Results
   - Results files from scripts
     #### /Results/Figures_Tables
   - Figure outputs
   - Table Outputs
     #### /Results/Model_Stats_Output
   - Saved model and stat outputs
